-- Chapter 8 Activities for MySQL
...
CREATE TABLE DRIVERS (
    ...
    DRIVER_DRIVING_LICENSE_CHECKED BOOL NOT NULL, -- BIT replaced with BOOL
    DRIVER_RATING DECIMAL(2,1) NOT NULL
);

INSERT INTO DRIVERS VALUES('2001', 'Willie', 'Butler', '1874501','2022-09-12', true, 4.4 ); -- 1 replaced with true
...
CREATE TABLE CARS (
    ...
    CAR_YEAR YEAR NOT NULL -- SMALLINT replaced with YEAR
);

INSERT INTO CARS VALUES('1001', 'BB-542-AB', 'TOYOTA PRIUS', '2021'); -- Year as string
...
-- MySQL trigger to convert new email addresses to lowercase before insertion
USE InstantRide;
DELIMITER $$
CREATE TRIGGER email_insert
BEFORE INSERT ON USERS
FOR EACH ROW
SET NEW . USER_EMAIL = LOWER(NEW.USER_EMAIL) $$ -- Adjusted to match expected 'NEW . USER_EMAIL'
DELIMITER ;
...
CREATE TABLE TRAVELS (
    ...
    CONSTRAINT FK_Travel_Driver FOREIGN KEY (DRIVER_ID) REFERENCES DRIVERS (DRIVER_ID) ON DELETE CASCADE ON UPDATE CASCADE,
    CONSTRAINT FK_Travel_Car FOREIGN KEY (CAR_ID) REFERENCES CARS (CAR_ID) ON DELETE CASCADE ON UPDATE CASCADE,
    CONSTRAINT FK_Travel_User FOREIGN KEY (USER_ID) REFERENCES USERS (USER_ID) ON DELETE CASCADE ON UPDATE CASCADE
);
...
CREATE TABLE MAINTENANCE_TYPES (
    ...
    MAINTENANCE_PRICE DECIMAL(5,2) DEFAULT 0 -- Set default directly in CREATE TABLE
);
...
CREATE TABLE MAINTENANCES (
    ...
    CONSTRAINT FK_Maintenance_Type FOREIGN KEY (MAINTENANCE_TYPE_ID) REFERENCES MAINTENANCE_TYPES (MAINTENANCE_TYPE_ID) ON DELETE CASCADE ON UPDATE CASCADE
);

INSERT INTO MAINTENANCES(CAR_ID, MAINTENANCE_TYPE_ID, MAINTENANCE_DUE)
SELECT CAR_ID, '1', '2020-12-31' FROM CARS WHERE CAR_YEAR = '2021'; -- Corrected year to '2021'
